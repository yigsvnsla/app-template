...
export function UserForm({ userImpersonated }: UserCreateIndexProps) {
	const user = userImpersonated?.user;
	const session = userImpersonated?.session;
	...
	const form = useRemixForm<z.infer<typeof CreateUserFormSchema>>({
		...
		defaultValues: {
			name: user?.name ?? "First Name Last Name",
			email: user?.email ?? `${crypto.randomUUID().split("-")[0]}@example.com`,
			role: user?.role ?? "user",
			password: "123456789",
		},
		submitHandlers: {
